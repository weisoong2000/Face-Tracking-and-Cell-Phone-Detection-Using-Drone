...
def trackFace(me, img, info, w, h, pid, DetectRange, pErrorRotate, pErrorUp):
    area = info[1]
    x, y = info[0][0], info[0][1]   # info[] from the findface output
    fb = 0
    ErrorRotate = x - w/2
    ErrorUp = h/2 - y
...
while con1:
    ...
    spErrorRotate, pErrorUp = trackFace(Drone, img, info, Camera_Width, Camera_Height, PID_Parameter, DetectRange,
                                        pErrorRotate, pErrorUp)
    classIds, confs, bbox = net.detect(OriginalImage, confThreshold=thres,
                                       nmsThreshold=nmsThres)  # To remove duplicates / declare accuracy
    print('face track started!')
    img = Drone.get_frame_read().frame
    classIds, confs, bbox = net.detect(img, confThreshold=thres,
                                       nmsThreshold=nmsThres)  # To remove duplicates / declare accuracy
